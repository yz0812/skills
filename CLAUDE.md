...
Claude Code Skills 集合项目。当前采用混合形态 `ac`：`ac-plugin/` 中集中承载 7 个仅手动触发的 skill 实现，`.claude/skills/ac-*` 作为兼容入口保留 `/ac-plan`、`/ac-execute`、`/ac-debug`、`/ac-init`、`/ac-review`、`/ac-report`、`/ac-commit` 命令；其余独立 skill 如 `mindmap` 按原方式保留。多个 ac skill 采用“主线程 + 按需 subagents 辅助”的执行模式。
...
| 形态 | 入口 | 触发方式 | MCP 依赖 |
|-------|------|----------|----------|
| ac plugin + wrappers | `.claude/skills/ac-*` + `ac-plugin/.claude-plugin/plugin.json` + `ac-plugin/skills/*` | `/ac-plan`、`/ac-execute`、`/ac-debug`、`/ac-init`、`/ac-review`、`/ac-report`、`/ac-commit` | `ac-report` 可选 `git-server`；其余按各自流程执行，其中多个 skill 支持主线程统筹 + 按需 subagents 辅助 |
